...
export default function Home() {
  ...
  const getCardStyle = (cardName) => {
    ...
    if (hoveredCard === cardName) {
      return { transform: "scale(1.04) translateY(-4px)", zIndex: 10 };
    }
    return { transform: "scale(0.97)", zIndex: 1 };
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-tr from-[#eef2ff] via-[#f5f3ff] to-[#faf5ff] text-slate-900 font-sans antialiased relative flex flex-col justify-between overflow-x-hidden selection:bg-purple-200">
      ...
      <header className="max-w-5xl mx-auto w-full pt-8 px-6 flex items-center justify-between relative z-10">
        <div className="flex items-center space-x-2 bg-white/75 backdrop-blur-md border border-white/80 px-3.5 py-1.5 rounded-2xl shadow-xs transition-transform duration-300 hover:scale-105">
      ...
      {/* MAIN CONTAINER BOX */}
      <div className="max-w-5xl mx-auto w-full px-6 my-auto py-4 relative z-10 flex flex-col justify-center flex-grow">
        
        {/* HERO STATEMENT */}
        <div className="max-w-2xl mx-auto pb-8 -mt-3 text-center space-y-3">
          <h1 className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tight leading-tight flex items-center justify-center gap-3">
            <span className="inline-block text-emerald-600 animate-[bounce_2s_infinite] text-4xl">🍃</span>
        ...
        <main className="grid grid-cols-1 md:grid-cols-12 gap-6 w-full items-stretch">
          ...
          <div 
            ...
            className="md:col-span-6 bg-white/80 backdrop-blur-xl border border-white/80 rounded-3xl p-7 sm:p-8 shadow-sm hover:shadow-2xl hover:border-purple-200 transition-transform duration-500 ease-out flex flex-col justify-between group"
          >
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <span className="w-11 h-11 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-center text-xl shadow-xs group-hover:scale-110 transition-transform duration-500">
                  📈
                </span>
              ...
              <div className="space-y-2">
                <h2 className="text-xl font-black text-slate-900 tracking-tight">{current.priceTitle}</h2>
            ...
            <button
              ...
              className="mt-8 w-full bg-slate-900 hover:bg-black text-white font-bold text-xs py-3.5 px-4 rounded-xl transition-all duration-300 tracking-wide shadow-sm hover:shadow-md active:scale-[0.98]"
            >
              {current.priceBtn}
            </button>
          </div>
          ...
          >
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <span className="w-11 h-11 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-xl shadow-xs group-hover:scale-110 transition-transform duration-500">
                  🍃
                </span>
              ...
              <div className="space-y-2">
                <h2 className="text-xl font-black text-slate-900 tracking-tight">{current.leafTitle}</h2>
                <p className="text-slate-600 text-xs sm:text-sm leading-relaxed font-medium">
                  {current.leafDesc}
                </p>
              </div>
            ...
            <button
              ...
              className="mt-8 w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-3.5 px-4 rounded-xl transition-all duration-300 tracking-wide shadow-sm hover:shadow-md active:scale-[0.98]"
            >
              {current.leafBtn}
            </button>
          </div>
      ...
      {/* FOOTER */}
      <footer className="border-t border-purple-100/60 py-6 px-6 bg-white/40 backdrop-blur-md relative z-10">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between text-[11px] font-medium text-slate-500 gap-2">
